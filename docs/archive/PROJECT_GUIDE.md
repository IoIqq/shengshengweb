...
> ⚠️ **强制规范**: 所有开发者必须阅读并遵守 [代码规范文档](CODE_STANDARDS.md)
> 
> 🚨 **禁止在 `server/server.js` 中写入冗余代码！** 项目已完成模块化，必须按照模块化结构开发。
...
**详细规范请查看：[CODE_STANDARDS.md](CODE_STANDARDS.md)**

### 重构成果

- ✅ 主服务器文件：3469行 → 192行（减少94.5%）
- ✅ 代码按功能清晰组织
- ✅ 便于维护和团队协作
...
│   │   ├── media/             # 素材文件
│   │   └── inbox/             # 待处理文件
│   ├── server.js              # 原服务器文件（3469行，待弃用）
│   └── server-new.js          # 新服务器入口（192行）
│
├── 📁 assets/                 # 静态资源
│   └── ui/                    # UI 资源
│
...
> ⚠️ **重要**: 后端代码已完成模块化重构，新功能开发请遵循模块化结构。详见 [CODE_STANDARDS.md](CODE_STANDARDS.md)

### 入口文件
- **`server/server-new.js`** - 新版服务器入口（192行，推荐使用）
- ~~`server/server.js`~~ - 原服务器文件（3469行，待弃用）
...
#### 中间件（middleware/）
- `auth.js` - 认证中间件（requireAuth、requireAdmin等）
- `csrf.js` - CSRF保护
- `index.js` - 请求日志、错误处理
