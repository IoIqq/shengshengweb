...
export async function createBorrowRequest(formData) {
  ...
  if (!applicant || !deviceId || !purpose) {
    Toast.warning('请填写申请人、设备和借用目的');
    return;
  }
  ...
  try {
    ...
    const result = await requestJSON('/api/borrow-requests', {
      method: 'POST',
      body: { applicant, deviceId, purpose, expectedReturnAt },
    });

    // 添加到本地状态
    if (state.borrowCatalog) {
      state.borrowCatalog.push(result.request || result);
    }
...
export async function approveBorrowRequest(id, status) {
  try {
    setPending(true);
    const result = await requestJSON(`/api/borrow-requests/${id}/approve`, {
      method: 'POST',
      body: { status },
    });
    ...
    if (request) {
      Object.assign(request, result.request || { status });
    }
...
export async function returnBorrowRequest(id) {
  try {
    setPending(true);
    const result = await requestJSON(`/api/borrow-requests/${id}/return`, {
      method: 'POST',
      body: {},
    });
